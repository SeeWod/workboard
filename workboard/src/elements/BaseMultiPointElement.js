import {
  getBoundingRect,
  deepCopy,
  getRotatedPoint,
  getElementCenterPoint,
  isEqualArray
} from '../utils'
import BaseElement from './BaseElement'

import { transformPointOnElement } from '../utils'
import {checkIsAtMultiSegment, checkIsAtWherePoint} from '../utils/checkHit'
import { drawPointsFrame } from '../utils/draw'

// 基础多个点的组件的元素类
export default class BaseMultiPointElement extends BaseElement {
  constructor(opts = {}, app) {
    super(opts, app)
    this.app = app
console.log(opts)
    this.type =  'baseMultiPointElement'
    this.points = opts.points || []
  }
  serialize(){
    return {
      type: this.type,
      points: [...this.points],
      style: {
        ...this.style
      }
    }
  }

  //渲染激活态-操作框
  renderFramework(){
    drawPointsFrame(this.app.ctx, this.points, this.app.cooManager.scale)
  }

  // 添加坐标，具有多个坐标数据的图形，如线段、自由线
  addPoint(x, y, ...args) {
    if (!Array.isArray(this.points)) {
      return
    }
    this.points.push([x, y, ...args])
    return this
  }

  //用于调整线段
  updatePoint(i, x, y) {
    this.points[i][0] = x
    this.points[i][1] = y
  }


  // 检测是否被击中
  isHitEle(x, y) {
    let rp = transformPointOnElement(x, y, this) 
    return checkIsAtMultiSegment(this.points, rp)
  }

  //点击激活元素的哪里
  hitWhere(x, y){
    let rp = transformPointOnElement(x, y, this)
    if(checkIsAtWherePoint(this.points, rp) != 'none'){
      return checkIsAtWherePoint(this.points, rp)
    }else if(checkIsAtMultiSegment(this.points, rp)){
      return "line"
    }else{
      return 'none'
    }
  }

  // 更新元素包围框，用于具有多个坐标数据的图形
  // updateMultiPointBoundingRect() {
  //   let rect = getBoundingRect(this.pointArr)
  //   this.x = rect.x
  //   this.y = rect.y
  //   this.width = rect.width
  //   this.height = rect.height
  //   return this
  // }

  // 更新虚拟坐标点
  // updateFictitiousPoint(x, y) {
  //   this.fictitiousPoint.x = x
  //   this.fictitiousPoint.y = y
  // }

  // 保存元素初始状态
  // saveState() {
  //   let { rotate, x, y, width, height, pointArr } = this
  //   this.startRotate = rotate
  //   this.startX = x
  //   this.startY = y
  //   this.startPointArr = deepCopy(pointArr)
  //   this.startWidth = width
  //   this.startHeight = height
  //   return this
  // }

  //移动元素
  move(ox, oy) {
    this.points = this.points.map(point => {
      return [point[0] + ox, point[1] + oy, ...point.slice(2)]
    })
    let { startX, startY } = this
    this.x = startX + ox
    this.y = startY + oy
    return this
  }

  // 更新元素包围框
  // updateRect(x, y, width, height) {
  //   let { startWidth, startHeight, startPointArr } = this
  //   // 获取收缩比例
  //   let scaleX = width / startWidth
  //   let scaleY = height / startHeight
  //   // 所有点位都进行同步缩放
  //   this.pointArr = startPointArr.map(point => {
  //     let nx = point[0] * scaleX
  //     let ny = point[1] * scaleY
  //     return [nx, ny, ...point.slice(2)]
  //   })
  //   // 放大后会偏移拖拽元素，所以计算一下元素的新包围框和拖拽元素包围框的差距，然后绘制时整体往回偏移
  //   let rect = getBoundingRect(this.pointArr)
  //   let offsetX = rect.x - x
  //   let offsetY = rect.y - y
  //   this.pointArr = this.pointArr.map(point => {
  //     return [point[0] - offsetX, point[1] - offsetY, ...point.slice(2)]
  //   })
  //   this.updatePos(x, y)
  //   this.updateSize(width, height)
  //   return this
  // }

  // 根据指定中心点旋转元素的各个点
  // rotateByCenter(rotate, cx, cy) {
  //   this.pointArr = this.startPointArr.map(point => {
  //     let np = getRotatedPoint(point[0], point[1], cx, cy, rotate)
  //     return [np.x, np.y, ...point.slice(2)]
  //   })
  //   this.updateMultiPointBoundingRect()
  // }

  //获取图形应用了旋转之后的端点列表
  updateRotate(angle) {
    return this.pointArr.map(point => {
      let center = getElementCenterPoint(this)
      let np = getRotatedPoint(
        point[0],
        point[1],
        center.x,
        center.y,
        this.rotate
      )

    })
  }

  isExpired(){
    if(isEqualArray(this.points[0],this.points[this.points.length - 1])){
      return true
    }
    return false
  }


}
